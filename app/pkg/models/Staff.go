...
func (r *staffR) GetStores() StoreSlice {
	if r == nil {
		return nil
	}
	return r.Stores
...
var (
	staffAllColumns            = []string{"staff_id", "name", "cognito_user_id"}
...
func (o *Staff) Stores(mods ...qm.QueryMod) storeQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`store_staff` on `stores`.`store_id` = `store_staff`.`store_id`"),
		qm.Where("`store_staff`.`staff_id`=?", o.StaffID),
	)
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (staffL) LoadStores(ctx context.Context, e boil.ContextExecutor, singular bool, maybeStaff interface{}, mods queries.Applicator) error {
	var slice []*Staff
	var object *Staff
	...
	query := NewQuery(
		qm.Select("`stores`.`store_id`, `stores`.`company_id`, `stores`.`store_name`, `stores`.`address`, `stores`.`phone_number`, `stores`.`business_hours`, `a`.`staff_id`"),
		qm.From("`stores`"),
		qm.InnerJoin("`store_staff` as `a` on `stores`.`store_id` = `a`.`store_id`"),
		qm.WhereIn("`a`.`staff_id` in ?", args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Uses the global database handle.
func (o *Staff) AddStoresG(ctx context.Context, insert bool, related ...*Store) error {
...
// Appends related to o.R.Stores.
// Sets related.R.Staffs appropriately.
func (o *Staff) AddStores(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Store) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into `store_staff` (`staff_id`, `store_id`) values (?, ?)"
		values := []interface{}{o.StaffID, rel.StoreID}
...
// staff replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Staffs's Stores accordingly.
// Replaces o.R.Stores with related.
// Sets related.R.Staffs's Stores accordingly.
// Uses the global database handle.
func (o *Staff) SetStoresG(ctx context.Context, insert bool, related ...*Store) error {
...
// Replaces o.R.Stores with related.
// Sets related.R.Staffs's Stores accordingly.
func (o *Staff) SetStores(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Store) error {
	query := "delete from `store_staff` where `staff_id` = ?"
	values := []interface{}{o.StaffID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	removeStoresFromStaffsSlice(o, related)
	if o.R != nil {
		o.R.Stores = nil
	}
...
// Uses the global database handle.
func (o *Staff) RemoveStoresG(ctx context.Context, related ...*Store) error {
...
// Removes related items from R.Stores (uses pointer comparison, removal does not keep order)
// Sets related.R.Staffs.
func (o *Staff) RemoveStores(ctx context.Context, exec boil.ContextExecutor, related ...*Store) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from `store_staff` where `staff_id` = ? and `store_id` in (%s)",
...
func removeStoresFromStaffsSlice(o *Staff, related []*Store) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
		for i, ri := range rel.R.Staffs {
			if o.StaffID != ri.StaffID {
				continue
			}
