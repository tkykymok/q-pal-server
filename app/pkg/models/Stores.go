...
func (r *storeR) GetMenuSetDetails() MenuSetDetailSlice {
	...
	return r.MenuSetDetails
}

func (r *storeR) GetMenuSets() MenuSetSlice {
	if r == nil {
		return nil
	}
	return r.MenuSets
...
func (r *storeR) GetReservationMenus() ReservationMenuSlice {
	if r == nil {
		return nil
	}
...
func (o *Store) ActiveStaffs(mods ...qm.QueryMod) activeStaffQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`active_staffs`.`store_id`=?", o.StoreID),
	)
...
func (o *Store) MenuSetDetails(mods ...qm.QueryMod) menuSetDetailQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`menu_set_details`.`store_id`=?", o.StoreID),
	)

	return MenuSetDetails(queryMods...)
}

// MenuSets retrieves all the menu_set's MenuSets with an executor.
func (o *Store) MenuSets(mods ...qm.QueryMod) menuSetQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`menu_sets`.`store_id`=?", o.StoreID),
	)
...
func (o *Store) ReservationMenus(mods ...qm.QueryMod) reservationMenuQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`reservation_menus`.`store_id`=?", o.StoreID),
	)
...
func (storeL) LoadActiveStaffs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeStore interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`active_staffs.store_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load active_staffs")
	}

	var resultSlice []*ActiveStaff
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice active_staffs")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on active_staffs")
	}
	if err = results.Err(); err != nil {
...
// LoadMenuSetDetails allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (storeL) LoadMenuSetDetails(ctx context.Context, e boil.ContextExecutor, singular bool, maybeStore interface{}, mods queries.Applicator) error {
	var slice []*Store
	var object *Store
	...
	query := NewQuery(
		...
		qm.WhereIn(`menu_set_details.store_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.MenuSetDetails = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.StoreID == foreign.StoreID {
				local.R.MenuSetDetails = append(local.R.MenuSetDetails, foreign)
				if foreign.R == nil {
					foreign.R = &menuSetDetailR{}
				}
				foreign.R.Store = local
...
// LoadMenuSets allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (storeL) LoadMenuSets(ctx context.Context, e boil.ContextExecutor, singular bool, maybeStore interface{}, mods queries.Applicator) error {
	var slice []*Store
	var object *Store
	...
	query := NewQuery(
		qm.From(`menu_sets`),
		qm.WhereIn(`menu_sets.store_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice menu_sets")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.StoreID, foreign.StoreID) {
				local.R.MenuSets = append(local.R.MenuSets, foreign)
				if foreign.R == nil {
					foreign.R = &menuSetR{}
				}
				foreign.R.Store = local
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (storeL) LoadReservationMenus(ctx context.Context, e boil.ContextExecutor, singular bool, maybeStore interface{}, mods queries.Applicator) error {
	var slice []*Store
	var object *Store
	...
	query := NewQuery(
		...
		qm.WhereIn(`reservation_menus.store_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// of the store, optionally inserting them as new records.
// Appends related to o.R.MenuSetDetails.
// Sets related.R.Store appropriately.
// Uses the global database handle.
func (o *Store) AddMenuSetDetailsG(ctx context.Context, insert bool, related ...*MenuSetDetail) error {
	return o.AddMenuSetDetails(ctx, boil.GetContextDB(), insert, related...)
}

// AddMenuSetDetails adds the given related objects to the existing relationships
// of the store, optionally inserting them as new records.
// Appends related to o.R.MenuSetDetails.
// Sets related.R.Store appropriately.
func (o *Store) AddMenuSetDetails(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*MenuSetDetail) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.StoreID = o.StoreID
			...
			values := []interface{}{o.StoreID, rel.SetID, rel.StoreID, rel.MenuID}
	...
	if o.R == nil {
		o.R = &storeR{
			MenuSetDetails: related,
		}
	} else {
		o.R.MenuSetDetails = append(o.R.MenuSetDetails, related...)
	}
...
// AddMenuSetsG adds the given related objects to the existing relationships
// of the store, optionally inserting them as new records.
// Appends related to o.R.MenuSets.
// Sets related.R.Store appropriately.
// Uses the global database handle.
func (o *Store) AddMenuSetsG(ctx context.Context, insert bool, related ...*MenuSet) error {
	return o.AddMenuSets(ctx, boil.GetContextDB(), insert, related...)
}

// AddMenuSets adds the given related objects to the existing relationships
// of the store, optionally inserting them as new records.
// Appends related to o.R.MenuSets.
// Sets related.R.Store appropriately.
func (o *Store) AddMenuSets(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*MenuSet) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.StoreID, rel.SetID}
	...
	if o.R == nil {
		...
		o.R.MenuSets = append(o.R.MenuSets, related...)
	}
...
// SetMenuSetsG removes all previously related items of the
// store replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Store's MenuSets accordingly.
// Replaces o.R.MenuSets with related.
// Sets related.R.Store's MenuSets accordingly.
// Uses the global database handle.
func (o *Store) SetMenuSetsG(ctx context.Context, insert bool, related ...*MenuSet) error {
	return o.SetMenuSets(ctx, boil.GetContextDB(), insert, related...)
}

// SetMenuSets removes all previously related items of the
// store replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Store's MenuSets accordingly.
// Replaces o.R.MenuSets with related.
// Sets related.R.Store's MenuSets accordingly.
func (o *Store) SetMenuSets(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*MenuSet) error {
	query := "update `menu_sets` set `store_id` = null where `store_id` = ?"
	values := []interface{}{o.StoreID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	return o.AddMenuSets(ctx, exec, insert, related...)
}

// RemoveMenuSetsG relationships from objects passed in.
// Removes related items from R.MenuSets (uses pointer comparison, removal does not keep order)
// Sets related.R.Store.
// Uses the global database handle.
func (o *Store) RemoveMenuSetsG(ctx context.Context, related ...*MenuSet) error {
	return o.RemoveMenuSets(ctx, boil.GetContextDB(), related...)
}

// RemoveMenuSets relationships from objects passed in.
// Removes related items from R.MenuSets (uses pointer comparison, removal does not keep order)
// Sets related.R.Store.
func (o *Store) RemoveMenuSets(ctx context.Context, exec boil.ContextExecutor, related ...*MenuSet) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.MenuSets {
			...
			ln := len(o.R.MenuSets)
			if ln > 1 && i < ln-1 {
				o.R.MenuSets[i] = o.R.MenuSets[ln-1]
			}
			o.R.MenuSets = o.R.MenuSets[:ln-1]
			break
		}
	}
...
func (o *Store) AddMenus(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Menu) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.StoreID, rel.StoreID, rel.MenuID}
...
// Sets related.R.Store appropriately.
// Uses the global database handle.
func (o *Store) AddReservationMenusG(ctx context.Context, insert bool, related ...*ReservationMenu) error {
...
// Appends related to o.R.ReservationMenus.
// Sets related.R.Store appropriately.
func (o *Store) AddReservationMenus(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ReservationMenu) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.StoreID = o.StoreID
			...
			values := []interface{}{o.StoreID, rel.ReservationID, rel.StoreID, rel.MenuID}
